...
static int set_smt_state(int smt_state) {
	...
	for (i = 0; i < num_present_cores; i++) {
		...
		int cpu_base = core * threads_per_cpu;
...
static int do_cores_on(char *state) {
	...
	if (new_state) {
		for (i = 0; i < cpus_in_system; i++) {
			int core = present_cores[i];
			if (!core_state[i]) {
				rc = set_one_core(smt_state, core, new_state);
				if (!rc) {
					number_changed++;
				}
	...
	if (number_changed != number_to_change) {
		...
		for (i = 0; i < cpus_in_system; i++) {
			int core = present_cores[i];
			if (cpu_online(core * threads_per_cpu)) {
				cores_now_online++;
			}
		}
...
static int do_info(void) {
	...
	int rc = get_present_core_list(&core_list, &num_cores, threads_per_cpu);
	if (rc != 0) {
		fprintf(stderr, "Failed to retrieve present core list\n");
		return rc;
	}
	...
	for (i = 0; i < num_cores; i++) {
		...
		for (j = 0; j < threads_per_cpu; j++) {
			thread_num = core * threads_per_cpu + j;
